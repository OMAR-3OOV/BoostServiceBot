...
public class TicketManagerEvent extends ListenerAdapter {
    ...
    public void onButtonClick(@NotNull ButtonClickEvent event) {
        ...
        try {
            ...
            if (Objects.requireNonNull(Objects.requireNonNull(event.getButton()).getId()).equalsIgnoreCase("open-ticket")) {
                ...
                TextChannel textChannel = Objects.requireNonNull(guildManager.getTicketCategory()).createTextChannel("ticket").complete();
            ...
            if (TicketManagerCommand.Message.containsKey(user)) {
                if (Objects.requireNonNull(Objects.requireNonNull(event.getButton()).getId()).equalsIgnoreCase("ticket-controlpanel-button-channelchange-" + user.getId())) {
                    ...
                    buttons.addAll(event.getInteraction().getMessage().getButtons().stream().filter(f -> !f.getId().equals(event.getButton().getId())).toList());
    ...
    public void onTextChannelUpdateParent(@NotNull TextChannelUpdateParentEvent event) {
        try {
            ...
            if (event.getChannel().getId().equalsIgnoreCase(guildManager.getTicketChannel().getId()) && event.getOldParent().getId().equalsIgnoreCase(guildManager.getTicketCategory().getId())) {
                guildManager.setCategoryTicket(event.getNewParent().getId());
    ...
    public void onTextChannelDelete(@NotNull TextChannelDeleteEvent event) {
        try {
            ...
            event.getGuild().retrieveAuditLogs().type(ActionType.CHANNEL_DELETE).limit(1).queue(list -> {
                try {
                    ...
                    if (event.getChannel().getId().equalsIgnoreCase(guildManager.getTicketChannelId())) {
                        ...
                        embed.setFooter(languageManager.getMessage(MessageKeys.PRIVATE_WARN_CHANNEL_OWNERSHIP_FLOOR).replaceAll("<time>", new SimpleDateFormat("HH:mm:ss dd - MMM - yyyy").format(new Date()))).setTimestamp(Instant.now());

                        channel.queue(msg -> msg.sendMessageEmbeds(embed.build()).queue());
                    }

                    TicketCreateUtility ticketsCreate = new TicketCreateUtility(event.getChannel(), event.getGuild());
                    ...
                    if (ticketsCreate.getFile() != null && ticketsCreate.getFile().exists()) {
                        ...
                        if (ticketsCreate.getFile().delete()) {
                            ...
                            if (closeTicketCommand.ticketchannel.containsKey(creator)) {
                                ...
                                cembed.setFooter(languageManager.getMessage(MessageKeys.PRIVATE_TICKET_CREATOR_CLOSE_EMBED_FLOOR).replaceAll("<time>", new SimpleDateFormat("HH:mm:ss dd - MMM - yyyy").format(new Date()))).setTimestamp(Instant.now());
                                ...
                                embed2.setFooter(languageManager.getMessage(MessageKeys.PRIVATE_TICKET_CREATOR_DELETED_EMBED_FLOOR).replaceAll("<time>", new SimpleDateFormat("HH:mm:ss dd - MMM - yyyy").format(new Date()))).setTimestamp(Instant.now());
        ...
        try {
            ...
            if (detector_channelchange.containsKey(user) && guildManager.getSetup()) {
                try {
                    ...
                    Message msg = textChannel.sendMessageEmbeds(ticketUtility.embedTicketMessage().build()).setActionRow(Button.primary("open-ticket", "Open ticket")).complete();
